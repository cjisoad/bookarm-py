"""Manual teaching GUI for BookSeparatorArm.

Run on Ubuntu with:

    python3 scripts/separator/bookseparator_teach_gui.py

The script works with the real `bookseparator_control_py` package when it is
installed. If the package cannot be imported, the GUI still starts in a small
simulation mode so the interface and JSON export can be exercised.
"""
...
try:  # pragma: no cover - depends on external package installation
    from bookseparator_control_py import BookSeparatorArm as RealBookSeparatorArm
except Exception as exc:  # pragma: no cover - handled at runtime
    RealBookSeparatorArm = None
    IMPORT_ERROR = exc
...
DEFAULT_SERIAL_PORT = "/dev/bookseparator"
...
class DummyArm:
    ...
    def __init__(self) -> None:
        ...
        self.q_rad = [0.0, 0.0, 0.0, 0.0]
        self.raw_pos = [2048, 2048, 2048, 2048, 2048]
    ...
    def move_raw_positions(
        ...
        *,
        ...
        self.raw_pos = [int(x) for x in positions]
        return {"ok": True, "simulated": True, "mode": "raw", "raw_pos": self.raw_pos}
...
class DirectFirmwareArm:
    """Direct newline-delimited JSON backend matching bookseparator-firmware."""
    ...
    def connect_serial(self, *, port: str) -> "DirectFirmwareArm":
        try:
            import serial
        except ImportError as exc:
            raise RuntimeError(
                "缺少 pyserial，请在启动 GUI 的同一个环境安装："
    ...
    def _remember_line(self, line: str) -> None:
        ...
        del self.last_lines[:-8]

    def send(self, command: dict[str, Any]) -> None:
        if not self.connected:
            raise RuntimeError("串口尚未打开")
        payload = json.dumps(command, ensure_ascii=False, separators=(",", ":")).encode("utf-8") + b"\n"
        self.serial.write(payload)
        self.serial.flush()

    def read_json(self, timeout: float) -> dict[str, Any]:
        if not self.connected:
            raise RuntimeError("串口尚未打开")
        ...
        while time.monotonic() < deadline:
            ...
            if isinstance(value, dict):
                return value
        raise TimeoutError(f"未收到 JSON；最近串口行：{self.last_lines}")
    ...
    def request(
        ...
        *,
        ...
        while time.monotonic() < deadline:
            ...
            try:
                response = self.read_json(remaining)
            except TimeoutError:
                break
            ...
            return response
        raise TimeoutError(
            f"没有收到匹配响应 expected_t={expected_t}，最后响应={last_response}，最近串口行={self.last_lines}"
        )

    def probe_connection(self, *, response_timeout: float = DEFAULT_RESPONSE_TIMEOUT) -> dict[str, Any]:
        return self.read_arm_feedback(response_timeout=response_timeout).raw

    def enable_torque(self, *, wait_response: bool = False, response_timeout: float | None = None) -> dict[str, Any] | None:
        timeout = DEFAULT_RESPONSE_TIMEOUT if response_timeout is None else response_timeout
        if wait_response:
            return self.request({"T": 102, "cmd": 1}, expected_t=1021, response_timeout=timeout)
        self.send({"T": 102, "cmd": 1})
        return None

    def disable_torque(self, *, wait_response: bool = False, response_timeout: float | None = None) -> dict[str, Any] | None:
        timeout = DEFAULT_RESPONSE_TIMEOUT if response_timeout is None else response_timeout
        if wait_response:
            return self.request({"T": 102, "cmd": 0}, expected_t=1021, response_timeout=timeout)
        self.send({"T": 102, "cmd": 0})
        return None
    ...
    def read_arm_feedback(self, *, response_timeout: float | None = None, validate_limits: bool = False) -> SimpleNamespace:
        ...
        response = self.request(
            ...
            response_filter=lambda item: "joints_rad" in item and "joints_torque" in item,
        )
        return SimpleNamespace(
            raw=response,
            q_rad=to_float_list(response.get("joints_rad")),
            torque=to_float_list(response.get("joints_torque")),
            raw_pos=to_int_list(response.get("raw_pos")),
            online=to_bool_list(response.get("online")),
        )
    ...
    def move_joints_rad(
        ...
        *,
        ...
        command = {
            "T": 103,
            "base": float(joint_angles_rad[0]),
            "shoulder": float(joint_angles_rad[1]),
            "elbow": float(joint_angles_rad[2]),
            "wrist": float(joint_angles_rad[3]),
            "spd": int(speed),
            "acc": int(acceleration),
        }
        timeout = DEFAULT_RESPONSE_TIMEOUT if response_timeout is None else response_timeout
        if wait_response:
            return self.request(command, expected_t=1031, response_timeout=timeout)
        self.send(command)
        return None
    ...
    def move_raw_positions(
        ...
        *,
        ...
        timeout = DEFAULT_RESPONSE_TIMEOUT if response_timeout is None else response_timeout
        if wait_response:
            return self.request(command, expected_t=1041, response_timeout=timeout)
        self.send(command)
        return None
    ...
    def set_gripper_closed(
        ...
        *,
        ...
        command_id = 112 if closed else 111
        expected_t = command_id * 10 + 1
        command = {
            "T": command_id,
            "spd": int(speed),
            "acc": int(acceleration),
        ...
        timeout = DEFAULT_RESPONSE_TIMEOUT if response_timeout is None else response_timeout
        if wait_response:
            return self.request(command, expected_t=expected_t, response_timeout=timeout)
        self.send(command)
        return None
...
class RealArm:
    ...
    def connect_serial(self, *, port: str) -> "RealArm":
        self.robot = RealBookSeparatorArm().connect_serial(port=port)
        return self
    ...
    def enable_torque(self, *, wait_response: bool = False, response_timeout: float | None = None) -> Any:
        return self.robot.enable_torque(wait_response=wait_response, response_timeout=response_timeout)

    def disable_torque(self, *, wait_response: bool = False, response_timeout: float | None = None) -> Any:
        return self.robot.disable_torque(wait_response=wait_response, response_timeout=response_timeout)

    def read_arm_feedback(self, *, response_timeout: float | None = None, validate_limits: bool = False) -> Any:
        return self.robot.read_arm_feedback(
    ...
    def move_raw_positions(
        ...
        *,
        ...
        return self.robot.move_raw_positions(
            positions,
            speed=speed,
            acceleration=acceleration,
            wait_response=wait_response,
...
class ArmTeachGUI:
    def __init__(self, root: tk.Tk, simulate: bool = False, ui_scale: float = DEFAULT_UI_SCALE) -> None:
        ...
        self.backend_mode_var = tk.StringVar(value="固件直连")
        ...
        self.feedback_var = tk.StringVar(value="q=- | raw=-")
    ...
    def _build_ui(self) -> None:
        ...
        columns = ("name", "gripper", "q0", "q1", "q2", "q3", "created")
        ...
        self._log("GUI 已启动")
        if IMPORT_ERROR is not None and not self.simulate_requested:
    ...
    def _make_backend(self, *, mode: str, baudrate: int, dtr: bool, rts: bool) -> Any:
        if self.simulate_requested or mode == "模拟":
            return DummyArm()
        if mode == "控制库":
            if RealBookSeparatorArm is None:
                raise RuntimeError(f"控制库不可用：{IMPORT_ERROR}")
            return RealArm()
        return DirectFirmwareArm(
            baudrate=baudrate,
            dtr=dtr,
            rts=rts,
        )
    ...
    def _handle_event(self, kind: str, payload: Any) -> None:
        if kind == "log":
            ...
            self.feedback_var.set(f"q={display_joint_values(q_rad)} | raw={display_int_values(raw_pos)}")
    ...
    def on_connect(self) -> None:
        ...
        def task() -> None:
            ...
            if isinstance(backend, DummyArm):
                label = "模拟"
            elif isinstance(backend, DirectFirmwareArm):
                label = f"固件直连 {backend.baudrate}"
            else:
                label = "控制库"
            ...
            if isinstance(backend, DirectFirmwareArm):
                try:
                    ack = backend.probe_connection(response_timeout=timeout)
                    self._post("log", f"通讯自检通过：{ack}")
                except Exception as exc:
                    self._post("log", f"通讯自检失败：{exc}")
            elif isinstance(backend, RealArm):
    ...
    def on_probe_connection(self) -> None:
        ...
        def task() -> None:
            ...
            if isinstance(backend, DirectFirmwareArm):
                with self.io_lock:
                    response = backend.probe_connection(response_timeout=settings.response_timeout)
                self._post("log", f"通讯自检响应：{response}")
            else:
    ...
    def on_gripper_toggle(self) -> None:
        ...
        def task() -> None:
            ...
            self._post("log", "爪夹已闭合" if closed else "爪夹已打开")
    ...
    def on_record_point(self) -> None:
        ...
        def task() -> None:
            ...
            raw_pos = to_int_list(getattr(fb, "raw_pos", None))
            online = to_bool_list(getattr(fb, "online", None))
            if q_rad is None or len(q_rad) != 4:
                raise ValueError("机械臂反馈中没有 4 个关节角")
            point = TeachPoint(
                name=name,
                q_rad=q_rad,
                created_at=now_iso(),
    ...
    def on_export_json(self) -> None:
        if not self.points:
            ...
            return
        default_name = f"bookseparator_points_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        backend = self.backend
        payload = {
            "schema": "bookseparator.manual_teach.v1",
            "exported_at": now_iso(),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="BookSeparator manual teaching GUI")
    ...
    if app.simulate_requested and RealBookSeparatorArm is not None:
        app._log("已强制进入模拟模式")
    root.mainloop()
    return 0
